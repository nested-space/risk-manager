...
async def test_material_add_augment_yes_fills_smiles_and_aliases(
    ...
    await dispatcher.advance_prompt("false")  # interpret_chemically (SMILES read-only)

    material = await _material(temp_env, "aspirin")
    assert material.smiles == "CC(=O)Oc1ccccc1C(=O)O"
    assert material.display_name == "aspirin"
...
@pytest.mark.integration
async def test_ncrm_add_augment_uses_name_then_manual_display_name(
    temp_env: Environment, monkeypatch: pytest.MonkeyPatch
) -> None:
    """NCRM augment keys off name; display_name is entered manually after."""
    _patch_resolve(
        monkeypatch,
        ResolveResult(
            name="methanol",
    ...
    async with get_db_session(temp_env) as session:
        entries = await NcrmLibrary.get_where(session, NcrmLibrary.name == "methanol")
...
@pytest.mark.integration
async def test_counterion_add_augment_creates_with_smiles_and_aliases(
    temp_env: Environment, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Counterion augment carries the resolved SMILES through the finish form."""
    _patch_resolve(
        monkeypatch,
        ResolveResult(
            name="chloride",
